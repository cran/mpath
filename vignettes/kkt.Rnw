...
\maketitle
Some technical details on the solution path are presented, supplementary to the publications \cite{wang2014zip, wang2014zinb}. 
...
\end{equation}
In the \textbf{\textsf{R}} package \textbf{mpath}, $\alpha_1, \alpha_2$ are labeled as \texttt{alpha.zero, alpha.count}, respectively; $\lambda_1, \lambda_2$ are labeled as \texttt{lambda.zero, lambda.count}, respectively. For an maximizer $\hat\Phi$ of $p\ell_{ZIP}(\Phi; y)$ if and only if the subdifferential of $p\ell_{ZIP}(\Phi; y)$ at $\hat\Phi$ is 0. Take derivatives:
...
The subdifferential of $p(\zeta, \beta)$ at $\hat\zeta_j\neq0$ is $n\alpha_1\lambda_1\text{sign}(\zeta_j)+\lambda_1(1-\alpha_1)\zeta_j$, and the subdifferential of $p(\zeta, \beta)$ at $\hat\beta_k\neq0$ is  $n\alpha_2\lambda_2\text{sign}(\beta_k)+\lambda_2(1-\alpha_2)\beta_k$.
The subdifferential of $p(\zeta, \beta)$ at $\hat\zeta_j=0$ is $n\alpha_1\lambda_1e_1$ for $e_1\in [-1,1]$, and the subdifferential of $p(\zeta, \beta)$ at $\hat\beta_k=0$ is $n\alpha_2\lambda_2e_2$ for $e_2\in [-1,1]$. Together, we have the KKT conditions for an maximizer $\hat\Phi$ of $p\ell_{ZIP}(\Phi; y)$:
...
\end{equation}
Denote $\lambda_{1, \max}$ and $\lambda_{2, \max}$ the smallest values of $\lambda_1$ and $\lambda_2$, respectivley, such that $\hat\zeta_j=0, \hat\beta_k=0$, and $(\lambda_{1, \max}, \lambda_{2, \max})$ can be determined by (\ref{eqn:lmax}) and the following quantities:
\begin{equation}
  \begin{aligned}
...
\begin{equation}\label{eqn:plzip}
  \begin{aligned}
      p\ell_{ZIP}^c(\Phi;y,z)=&\sum_{i=1}^n \left\{z_iG_i^\intercal\zeta - \log(1+\exp(G_i^\intercal\zeta))\right\}\\ 
    &+\sum_{i=1}^n(1-z_i)\left\{y_iB_i^\intercal\beta-\exp(B_i^\intercal\beta)-\log(y_i!)\right\}
      -p(\zeta, \beta).
  \end{aligned}
\end{equation}
...
(\ref{eqn:lmax1}). Furthermore, we have shown that $(\lambda_{1, \max}, \lambda_{2, \max})=(\hat\lambda_{1, \max}, \hat\lambda_{2, \max})$ holds.
...
The KKT conditions for an maximizer $\hat\Phi$ of $p\ell_{ZINB}(\Phi)$ can be derived. 
Therefore, for $\hat\zeta_j=0, \hat\beta_k=0$, it must be:
...
\end{equation}
Denote $\lambda_{1, \max}$ and $\lambda_{2, \max}$ the smallest values of $\lambda_1$ and $\lambda_2$, respectivley, such that $\hat\zeta_j=0, \hat\beta_k=0$, and $(\lambda_{1, \max}, \lambda_{2, \max})$ can be determined by (\ref{eqn:deri3}), (\ref{eqn:lmaxnb}) and the following quantities:
\begin{equation}
...
\begin{equation*}
    \begin{aligned}
    p\ell^c_{ZINB}(\Phi; y, z)=&\sum_{i=1}^n \left\{(z_iG_i^\intercal\zeta-\log(1+\exp(G_i^\intercal\zeta))+(1-z_i)\log(f(y_i;\beta, \theta))\right\}\\
  &-p(\zeta, \beta),
    \end{aligned}
\end{equation*}
...
\begin{equation}\label{eqn:deri4}
     ...
     \end{aligned}
\end{equation}
The KKT conditions of an maximizer $\hat\Phi$ for $\ell_{ZINB}^c(\Phi;y,z)$ can be derived. Therefore, for $\hat\zeta_j=0, \hat\beta_k=0$, it must be:
...
These connections offer a different method (see \citet{wang2014zinb}) to derive $(\hat\lambda_{1, \max}, \hat\lambda_{2, \max})$ such that $\hat\zeta_j=0, \hat\beta_k=0$ hold. We first estimate $\zeta_0, \beta_0$ for an
