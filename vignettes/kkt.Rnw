...
The subdifferential of $p(\zeta, \beta)$ at $\hat\zeta_j=0$ is $n\alpha_1\lambda_1e_1$ for $e_1\in [-1,1]$, and the subdifferential of $p(\zeta, \beta)$ at $\hat\beta_k=0$ is $n\alpha_2\lambda_2e_2$ for $e_2\in [-1,1]$. Together, we have the KKT conditions for a minimizer $\hat\Phi$ of $p\ell_{ZIP}(\Phi; y)$:
